...
const HOLIDAY_CHIPS = [
  { label: '🎄 Christmas wknd', dates: ['2025-12-24','2025-12-25','2025-12-26','2025-12-27','2025-12-28'], desc: 'Dec 24-28' },
  ...
  { label: '🌸 Spring Break', dates: ['2026-03-14','2026-03-15','2026-03-16','2026-03-17','2026-03-18','2026-03-19','2026-03-20','2026-03-21','2026-03-22'], desc: 'Mar 14-22' },
  ...
  const saveRange = useCallback(async (start: Date, end: Date, type: AvailabilityType) => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        trip_id: tripId,
        user_id: userId,
        start_date: format(start <= end ? start : end, 'yyyy-MM-dd'),
        end_date: format(start <= end ? end : start, 'yyyy-MM-dd'),
        type,
      })
      .select()
      .single()
    ...
    <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {HOLIDAY_CHIPS.map(chip => {
              ...
              return (
                <button
                  key={chip.label}
                  onClick={() => !added && handleChip(chip.dates)}
                  disabled={saving || added}
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm border transition ${added ? 'bg-green-50 border-green-200 text-green-700 cursor-default' : 'bg-white border-stone-200 text-stone-700 hover:border-stone-400 hover:shadow-sm'} disabled:opacity-60`}
                >
                  {chip.label}
                  <span className="text-xs text-stone-400">{chip.desc}</span>
                  {added && <span className="text-green-500 text-xs ml-1">✓</span>}
                </button>
              )
            })}
          </div>
        ...
        <div className="flex items-center gap-3 mb-6 flex-wrap">
          ...
          <div className="flex rounded-lg border border-stone-200 overflow-hidden text-sm">
            <button
              ...
              className={`px-4 py-1.5 font-medium transition ${mode === 'available' ? 'bg-green-600 text-white' : 'bg-white text-stone-500 hover:bg-stone-50'}`}
            >✓ Available</button>
            <button
              onClick={() => setMode('unavailable')}
              className={`px-4 py-1.5 font-medium border-l border-stone-200 transition ${mode === 'unavailable' ? 'bg-red-500 text-white' : 'bg-white text-stone-500 hover:bg-stone-50'}`}
            >✕ Busy</button>
          </div>
          <span className="ml-auto text-xs text-stone-400">
            {rangeStart ? 'Click end date' : 'Click start, then end'}
          </span>
        </div>

        <div className="flex items-center justify-between mb-4">
          <button onClick={() => setCurrentMonth(d => new Date(d.getFullYear(), d.getMonth() - 1))} className="w-8 h-8 flex items-center justify-center rounded-lg text-stone-400 hover:text-stone-900 hover:bg-stone-100 transition">←</button>
          ...
          {days.map(day => {
            ...
            return (
              <button
                ...
                className={`aspect-square rounded-lg text-sm flex items-center justify-center transition ${isToday(day) ? 'ring-2 ring-stone-300' : ''} ${isStart ? 'ring-2 ring-blue-400' : ''} ${bg}`}
              >
                {format(day, 'd')}
              </button>
            )
      ...
      {justSaved && (
        <div className="bg-green-50 border border-green-200 rounded-2xl p-4 flex items-center justify-between gap-4">
          ...
          >View trip <ArrowRight size={14} /></button>
        </div>
      )}
    </div>
  )
